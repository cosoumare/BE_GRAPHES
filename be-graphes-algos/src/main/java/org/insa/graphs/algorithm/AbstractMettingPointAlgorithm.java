 ...
 */
public abstract class AbstractMettingPointAlgorithm<Observer> {
     ...
     */
    protected AbstractMettingPointAlgorithm(AbstractInputData data) {
        this.data = data;
     ...
     */
    protected AbstractMettingPointAlgorithm(AbstractInputData data, ArrayList<Observer> observers) {
        this.data = data;
    ...
    public AbstractSolution[] run() {
        ...
        for (AbstractSolution AS : solution) {
        	AS.setSolvingTime(Duration.between(start, Instant.now()));
        }
        return solution;
    }
